...
double lastError = 0;
const int GOAL = 3500;
const unsigned char MAX_SPEED =165;
...
int PID(){
    ...
    int y=(constrain(MAX_SPEED +adjustment, 0, MAX_SPEED));
    return x;
    Serial.println(x);
  }
void forward(){
  ...
  if(mode == LOW){
   ...
   digitalWrite(LED_BUILTIN, LOW);
    if(sensorFlag == LOW){
      ...
      else if(( sensorValues[4] == 1000 || sensorValues[3] == 1000 ||  sensorValues[5] == 1000 || sensorValues[2] == 1000)&&(sensorValues[7]<1000 && sensorValues[6]< 1000 && sensorValues[1]< 1000 && sensorValues[0]< 1000 ))
      {
       ...
       else if (sensorValues[7]==1000 || sensorValues[6]== 1000 )
       { 
       ...
       else if (sensorValues[1]== 1000 || sensorValues[0]== 1000)
       {
        Serial.println("LEFT");
          int x=PID();
          left2(x);
